...
module.exports = {
  ...
  login: {
    body: Joi.object({ email, password: Joi.string().max(128).required() }),
  },
  refresh: {
    body: Joi.object({ refreshToken: Joi.string().max(512).required() }),
  },
  logout: {
    body: Joi.object({ refreshToken: Joi.string().max(512).required() }),
  },
  ...
  confirmPasswordReset: {
    body: Joi.object({ token: Joi.string().max(512).required(), password }),
  },
};
