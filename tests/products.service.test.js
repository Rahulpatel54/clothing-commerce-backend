...
function product(overrides = {}) {
  const row = {
    id: 'prod-1', name: 'Heavyweight Oversized Tee', slug: 'heavyweight-oversized-tee', sku: 'TEE-001',
    price: '1499.00', costPrice: '450.00', compareAtPrice: '1999.00', status: 'ACTIVE', categoryId: 'cat-1',
    tags: ['tee', 'oversized'], publishedAt: new Date('2026-01-01'),
    variants: [
  ...
  row.toPublicJSON = () => {
    ...
    json.variants = (json.variants || []).map((v) => { const copy = { ...v }; delete copy.costPrice; return copy; });
    return json;
  };
  return row;
}

beforeEach(() => { repo.transaction.mockImplementation(async (fn) => fn('tx')); });
...
describe('price presentation', () => {
  it('resolves variant price from the override, falling back to the parent', () => {
    ...
    expect(result.variants[0].effectivePrice).toBe(1499);
    expect(result.variants[1].effectivePrice).toBe(1599);
  });
...
describe('create', () => {
  ...
  it('rejects a duplicate variant SKU and rolls the transaction back', async () => {
    ...
    await expect(service.create(REQ, { name: 'Tee', sku: 'TEE-777', price: 100, variants: [{ sku: 'TEE-001-M-BLK', size: 'M', color: 'Black' }] })).rejects.toMatchObject({ statusCode: 409 });
  });

  it('rejects unknown collection ids', async () => {
    repo.findCollectionsByIds.mockResolvedValue([]);
    await expect(service.create(REQ, { name: 'Tee', sku: 'TEE-888', price: 100, collectionIds: ['c1', 'c2'] })).rejects.toMatchObject({ statusCode: 400 });
  });
});
