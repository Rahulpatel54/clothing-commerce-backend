...
function customer(overrides = {}) {
  const row = { id: 'cust-1', userId: 'user-1', totalSpend: '4500.00', orderCount: 3, marketingEmail: true, marketingSms: true, marketingWhatsapp: false, ...overrides };
  row.toJSON = () => ({ ...row });
...
describe('marketing preferences', () => {
  it('stamps an opt-out when every channel is switched off', async () => {
    repo.update.mockResolvedValue(undefined);
    await service.setMarketingPreferences(OWNER, 'cust-1', { marketingEmail: false, marketingSms: false, marketingWhatsapp: false });
  ...
  it('clears the opt-out when a channel is switched back on', async () => {
    repo.update.mockResolvedValue(undefined);
    await service.setMarketingPreferences(OWNER, 'cust-1', { marketingEmail: true, marketingSms: false, marketingWhatsapp: false });
