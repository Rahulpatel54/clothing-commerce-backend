...
beforeEach(() => {
  ...
  repo.revokeRefreshToken.mockImplementation(async (token, opts) =>
    Object.assign(token, { revokedAt: new Date(), revokedReason: opts.reason })
  );
});
...
describe('register', () => {
  ...
  it('rejects a duplicate email with 409', async () => {
    ...
    await expect(service.register({ email: 'shopper@example.com', password: 'Str0ngPass' }))
...
describe('login', () => {
  ...
  it('locks the account once the attempt limit is reached', async () => {
    user.failedLoginAttempts = 2; // limit is 3 in the test env
  ...
  it('gives the same error for an unknown email as for a wrong password', async () => {
    ...
    await expect(service.login({ email: 'nobody@example.com', password }))
      .rejects.toMatchObject({ statusCode: 401, message: 'Invalid email or password' });
  });
});
...
describe('refresh token rotation', () => {
  ...
  it('rotates: old token revoked and linked to its replacement', async () => {
    ...
    expect(repo.revokeRefreshToken).toHaveBeenCalledWith(
      record,
      expect.objectContaining({ reason: 'ROTATED', replacedByTokenId: 'rt-new' })
    );
  });
  ...
  it('rejects an expired refresh token without revoking the family', async () => {
    ...
    await expect(service.refresh({ refreshToken: raw }))
...
describe('password reset', () => {
  ...
  it('resets the password, consumes the token and revokes all sessions', async () => {
    ...
    expect(repo.revokeAllRefreshTokensForUser).toHaveBeenCalledWith(
      'user-1',
      expect.objectContaining({ reason: 'PASSWORD_RESET' })
    );
  });
  ...
  it('rejects a replayed reset token', async () => {
    ...
    await expect(service.confirmPasswordReset({ token: raw, password: 'N3wStrongPass' }))
      .rejects.toMatchObject({ statusCode: 400, message: 'This reset token has already been used' });
  });
  ...
  it('rejects an unknown reset token', async () => {
    ...
    await expect(service.confirmPasswordReset({ token: 'nope', password: 'N3wStrongPass' }))
      .rejects.toMatchObject({ statusCode: 400 });
  });
});
